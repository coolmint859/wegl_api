...
export default class ResourceCollector {
     ...
     * @param {string} resourcePath the path/url to the resource
     * @returns {boolean} true if the resource exists and was acquired, false otherwise
     */
    ...
    */
    static async #loadResource(resourcePath, loadFunction, options) {
        ...
        for (let loadAttempt = 0; loadAttempt <= maxRetries; loadAttempt++) {
            try {
                ...
                options.loadData.signal = compositeController.signal;
                const loadPromise = loadFunction(resourcePath, options.loadData);
    ...
    static async fetchImageFile(imagePath, options) {
        ...
        const response = await fetch(imagePath, { signal: options.signal });
        if (!response.ok) {
